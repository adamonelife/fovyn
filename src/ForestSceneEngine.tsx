...
import {useForestProjection} from './ForestEnvironmentScene';
...
export function ForestTreeLayer({assignments,trees,variant,onSelect,onHover,onLeave,debug=false}:{assignments:ForestAssignment[];trees:Record<string,ForestAsset>;variant:'hero'|'overview';environment:string;onSelect:(id:string)=>void;onHover?:(id:string)=>void;onLeave?:()=>void;debug?:boolean}){
  const{project}=useForestProjection();
  const className=variant==='hero'?'production-tree':'forest-overview-tree';
  return <>{assignments.map(({goal,slot})=>{const asset=trees[goal.id]??forestAssetFallback(goal.tree_asset_key);if(!asset)throw new Error(`Missing Tree asset ${goal.tree_asset_key}`);const point=project(slot);return <button key={goal.id} data-slot-id={slot.id} className={`${className} ${goal.health_state.toLowerCase().replaceAll(' ','-')}`} style={{left:point.x,top:point.y,'--slot-x':point.x+'px','--tree-scale':String(slot.scale*asset.default_scale),'--ground-anchor':String(asset.ground_anchor_y),'--z':String(slot.zIndex)} as CSSProperties} onMouseEnter={()=>onHover?.(goal.id)} onMouseLeave={onLeave} onFocus={()=>onHover?.(goal.id)} onBlur={onLeave} onClick={()=>onSelect(goal.id)} aria-label={`${goal.title}, ${treeLevelLabel(goal.tree_stage,false)}`}><span className="tree-asset-shell"><img src={asset.url} alt=""/><i/></span>{debug&&<span className="forest-tree-debug"><b>{slot.id}</b><i className="tree-ground-point"/><i className="slot-anchor-point"/><i className="anchor-link"/><em>Tree {(slot.x*100).toFixed(1)}, {(slot.y*100).toFixed(1)} · Slot {(slot.x*100).toFixed(1)}, {(slot.y*100).toFixed(1)}</em></span>}</button>})}</>;
}
...
export function ForestTreeLabels({assignments,environment,onSelect,onHover,onLeave}:{assignments:ForestAssignment[];environment:string;onSelect:(id:string)=>void;onHover:(id:string)=>void;onLeave:()=>void}){
  ...
  return <>{assignments.map(({goal,slot})=>{const placement=placements.find(item=>item.goalId===goal.id)!,point=project(placement),safeLeft=placement.anchor==='left'?`clamp(132px,${point.x}px,calc(100% - 8px))`:placement.anchor==='right'?`clamp(8px,${point.x}px,calc(100% - 132px))`:`clamp(70px,${point.x}px,calc(100% - 70px))`;return <button type="button" className={`forest-tree-label label-${placement.anchor} ${goal.health_state.toLowerCase().replaceAll(' ','-')}`} style={{left:safeLeft,top:point.y,'--label-offset-x':`${slot.labelOffsetX??0}px`,'--label-offset-y':`${slot.labelOffsetY??0}px`} as CSSProperties} onMouseEnter={()=>onHover(goal.id)} onMouseLeave={onLeave} onFocus={()=>onHover(goal.id)} onBlur={onLeave} onClick={()=>onSelect(goal.id)} key={goal.id}><b>{goal.title}</b><small>{treeLevelLabel(goal.tree_stage)}</small></button>})}</>;
}
...
export function ForestTreeCard({assignment,variant,onClose,onViewGoal,onLog,onMouseEnter,onMouseLeave,children}:{assignment:ForestAssignment;variant:'hero'|'overview';onClose:()=>void;onViewGoal:()=>void;onLog:()=>void;onMouseEnter?:()=>void;onMouseLeave?:()=>void;children?:ReactNode}){
  const{project}=useForestProjection(),{goal,slot}=assignment,base=variant==='hero'?'forest-tree-card':'forest-overview-card',point=project(slot);
  return <article className={`${base} contextual card-${resolvedCardDirection(slot)}`} style={{'--card-x':point.x+'px','--card-y':point.y+'px'} as CSSProperties} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}><button aria-label="Close Tree details" onClick={onClose}><X/></button><span>{goal.area_key.toUpperCase()}</span>{variant==='hero'?<h3>{goal.title}</h3>:<h2>{goal.title}</h2>}<p>{treeLevelLabel(goal.tree_stage)} · {goal.health_state}</p>{children??<small>{Math.round(goal.growth_consistency)}% Growth Consistency</small>}<div><button onClick={onViewGoal}>View Goal</button><button onClick={onLog}><Plus/> Log</button></div></article>;
}
