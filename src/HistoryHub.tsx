...
export default function HistoryHub({
  ...
  return (
    <PageContainer className="history-shell">
      <PageHeader
        eyebrow="HISTORY"
      ...
      />
      <PageTabs>
        {(["calendar", "timeline", "reviews"] as View[]).map((item) => (
          <button
            type="button"
      ...
      {view === "reviews" ? (
        ...
        <InsightsModule range={range} />
      ) : (
        <HistoryModule range={range} mode={view} openLog={openLog} />
      )}
    </PageContainer>
  );
}
