...
export async function loadSettings():Promise<SettingsData>{
  ...
  const[profile,areas,units,subcategories]=await Promise.all([
    supabase.from('profiles').select('first_name,last_name,display_name,username,date_of_birth,gender,country,preferred_language,timezone,unit_system,default_currency,week_starts_on,date_format,time_format,current_climate').eq('id',user.id).single(),
