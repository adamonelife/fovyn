...
import{loadRecovery,type RecoveryData}from'./recoveryRepository';
...
export default function RecoveryModule({query='',manage}:{query?:string;manage:()=>void}){
  const[data,setData]=useState<RecoveryData>({trackers:[],records:[],units:[]}),[selected,setSelected]=useState<Tracker>(),[loading,setLoading]=useState(true),[error,setError]=useState('');
  const load=async()=>{setLoading(true);try{setData(await loadRecovery());setError('')}catch(reason){setError(reason instanceof Error?reason.message:'Unable to load Supplements & Recovery')}finally{setLoading(false)}};
  useEffect(()=>{load()},[]);
  ...
  if(loading)return <div className="page-wrap tracker-loading">Loading Supplements & Recovery…</div>;
  return <div className="page-wrap recovery-v1">{error&&<p className="goal-error">{error}</p>}<LogSection title="Available to log">{visible.map(tracker=>{const latest=data.records.find(record=>record.tracker_id===tracker.id);return <LogItemCard key={tracker.id} icon={<HeartPulse/>} meta="Supplements & Recovery" title={tracker.name} detail={latest?`${latest.value} ${unit(tracker)} · ${new Date(latest.occurred_at).toLocaleDateString()}`:`${unit(tracker)||'Value'} · Not logged yet`} action={<span className="row-log">Log</span>} onClick={()=>setSelected(tracker)}/>})}{!visible.length&&<LogEmptyState icon={<HeartPulse/>} title={query?'No matching item':'Nothing configured yet'} detail={query?undefined:'Add a supplement, medication or recovery item under + Add & Manage.'}/>}</LogSection><button className="log-manage" onClick={manage}><Plus/> Add / Manage</button>{selected&&<Recorder tracker={selected} unit={unit(selected)} close={()=>setSelected(undefined)} saved={()=>{setSelected(undefined);load()}}/>}</div>;
}
