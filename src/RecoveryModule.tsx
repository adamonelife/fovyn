...
import {beginRecoveryProgramme,changeRecoveryStage,loadRecoveryHistory,loadRecoveryProgramme,updateRecoveryEnrolment,type RecoveryHistory,type RecoveryProgrammeData} from './recoveryProgrammeRepository';
...
function RecoveryProgrammePanel(){
  const[data,setData]=useState<RecoveryProgrammeData>(),[history,setHistory]=useState<RecoveryHistory>({sessions:[],sides:[]}),[stageId,setStageId]=useState(''),[busy,setBusy]=useState(false),[error,setError]=useState(''),[editingDetails,setEditingDetails]=useState(false),[programmeNotes,setProgrammeNotes]=useState(''),[prescribedBy,setPrescribedBy]=useState('self'),[professionalName,setProfessionalName]=useState('');
  const refresh=async()=>{try{const next=await loadRecoveryProgramme();setData(next);setStageId(next.enrolment?.current_stage_id??next.stages.find(stage=>stage.implemented)?.id??'');setProgrammeNotes(next.enrolment?.notes??'');if(next.enrolment)setHistory(await loadRecoveryHistory(next.enrolment.id));setError('')}catch(reason){setError(reason instanceof Error?reason.message:'Unable to load Recovery programmes')}};
  useEffect(()=>{refresh()},[]);
  ...
  return <div className="page-wrap recovery-programme">
    ...
    {data.enrolment&&<section className="recovery-history"><header><div><p className="eyebrow">RECOVERY HISTORY</p><h3>Recent workouts</h3></div><button className="soft-button" onClick={()=>setEditingDetails(!editingDetails)}>{editingDetails?'Close':'Programme details'}</button></header>{editingDetails&&<div className="recovery-programme-details"><label>Recommended by<select value={prescribedBy} onChange={event=>setPrescribedBy(event.target.value)}><option value="self">Self</option><option value="physiotherapist">Physiotherapist</option><option value="doctor">Doctor</option><option value="coach">Coach</option><option value="sports_therapist">Sports Therapist</option><option value="other">Other</option></select></label><label>Professional name (optional)<input value={professionalName} onChange={event=>setProfessionalName(event.target.value)}/></label><label className="wide">Programme notes<textarea value={programmeNotes} onChange={event=>setProgrammeNotes(event.target.value)}/></label><button className="save-record" disabled={busy} onClick={async()=>{setBusy(true);try{await updateRecoveryEnrolment(data.enrolment!.id,{notes:programmeNotes,prescribedBy,professionalName,professionalPrescribed:prescribedBy!=='self'});setEditingDetails(false);await refresh()}catch(reason){setError(reason instanceof Error?reason.message:'Unable to save programme details')}finally{setBusy(false)}}}>Save details</button></div>}{history.sessions.length?<div className="recovery-history-list">{history.sessions.map(session=><article key={session.id}><div><strong>{new Date(`${session.performed_on}T12:00:00`).toLocaleDateString()}</strong><span>{formatDisplayLabel(session.symptom_response??'not_recorded')}</span></div><p>{session.pain_before!=null||session.pain_after!=null?`Pain ${session.pain_before??'—'} → ${session.pain_after??'—'}`:'Pain not recorded'} · {session.stiffness_before!=null||session.stiffness_after!=null?`Stiffness ${session.stiffness_before??'—'} → ${session.stiffness_after??'—'}`:'Stiffness not recorded'}</p>{session.delayed_response&&<small>{formatDisplayLabel(session.delayed_response)}</small>}</article>)}</div>:<LogEmptyState icon={<HeartPulse/>} title="No Recovery workouts yet" detail="Completed Recovery workouts and symptom responses will appear here."/>}{history.sides.length>0&&<div className="recovery-side-summary"><h4>Side-to-side records</h4><p>{history.sides.filter(item=>item.side==='left').length} Left · {history.sides.filter(item=>item.side==='right').length} Right</p><small>Objective records only; Fovyn does not diagnose differences.</small></div>}</section>}
  </div>;
}
