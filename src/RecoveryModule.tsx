...
import{addMetricRecord,occurrenceValue,type OccurrenceStatus}from'./metricsRepository';
...
function Recorder({tracker,unit,statusMode,close,saved}:{tracker:Tracker;unit:string;statusMode:boolean;close:()=>void;saved:()=>void}){
  const[value,setValue]=useState(''),[status,setStatus]=useState<OccurrenceStatus>('complete'),[occurred,setOccurred]=useState(localNow()),[note,setNote]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false);
  return <div className="sheet-shade" onMouseDown={close}><section className="metric-recorder" onMouseDown={event=>event.stopPropagation()}><button className="sheet-close" onClick={close} aria-label="Close"><X/></button><p className="eyebrow">LOG</p><h2>{tracker.name}</h2>{statusMode&&<div className="occurrence-options" role="group" aria-label="What happened?">{(['complete','failed','skipped']as const).map(item=><button className={status===item?'active':''} onClick={()=>setStatus(item)} key={item}>{item==='complete'?'Done / Taken':item==='failed'?'Missed':'N/A'}</button>)}</div>}{(!statusMode||status==='complete')&&<label>{statusMode?'Dose or duration (optional)':'Value'}<div className="metric-value"><input autoFocus type="number" min="0" inputMode="decimal" step="any" value={value} onChange={event=>setValue(event.target.value)}/><span>{unit}</span></div></label>}<label>When<input type="datetime-local" value={occurred} onChange={event=>setOccurred(event.target.value)}/></label><label>Note (optional)<textarea value={note} onChange={event=>setNote(event.target.value)}/></label>{error&&<p className="goal-error">{error}</p>}<button className="save-record" disabled={busy||(!statusMode&&(value===''||!Number.isFinite(Number(value))))} onClick={async()=>{setBusy(true);try{await addMetricRecord(tracker,statusMode?occurrenceValue(status,value):Number(value),new Date(occurred).toISOString(),note,undefined,statusMode?status:null);saved()}catch(reason){setError(reason instanceof Error?reason.message:'We couldn’t save that entry. Please try again.')}finally{setBusy(false)}}}>{busy?'Saving…':'Log'}</button></section></div>;
}
...
export function TrackerCategoryModule({module,label,emptyDetail,Icon,query='',manage}:{module:TrackerCategory;label:string;emptyDetail:string;Icon:LucideIcon;query?:string;manage:()=>void}){
  ...
  return <div className="page-wrap recovery-v1">{error&&<p className="goal-error">{error}</p>}<LogSection title="Available to log">{visible.map(tracker=>{const latest=data.records.find(record=>record.tracker_id===tracker.id);return <LogItemCard key={tracker.id} icon={<Icon/>} meta={label} title={tracker.name} detail={latest?`${latest.occurrence_status?formatDisplayLabel(latest.occurrence_status):`${latest.value} ${unit(tracker)}`} · ${new Date(latest.occurred_at).toLocaleDateString()}`:`${unit(tracker)||'Value'} · Not logged yet`} action={<span className="row-log">Log</span>} onClick={()=>setSelected(tracker)}/>})}{!visible.length&&<LogEmptyState icon={<Icon/>} title={query?'No matching item':'Nothing configured yet'} detail={query?undefined:emptyDetail}/>}</LogSection><button className="log-manage" onClick={manage}><Plus/> Add / Manage</button>{selected&&<Recorder tracker={selected} unit={unit(selected)} statusMode={module==='medication'} close={()=>setSelected(undefined)} saved={()=>{setSelected(undefined);load()}}/>}</div>;
}
