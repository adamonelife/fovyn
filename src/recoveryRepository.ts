...
export const filterCategoryTrackers=(trackers:Tracker[],module:TrackerCategory)=>trackers.filter(tracker=>tracker.module===module&&tracker.status!=='archived');
export const filterRecoveryTrackers=(trackers:Tracker[])=>filterCategoryTrackers(trackers,'medication');

export async function loadTrackerCategory(module:TrackerCategory,label:string):Promise<TrackerCategoryData>{
  ...
  return{trackers:available,records:(records.data??[]).filter(record=>ids.has(record.tracker_id))as TrackerCategoryRecord[],units:options.units};
}

export const loadRecovery=()=>loadTrackerCategory('medication','Supplements & Recovery');
