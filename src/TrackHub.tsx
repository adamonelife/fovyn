...
type View='all'|'habits'|'metrics'|'activity'|'nutrition'|'sleep'|'notes'|'manage';
...
export default function TrackHub({onFirstSetupComplete}:{onFirstSetupComplete?:()=>Promise<void>|void}={}){
  ...
  const content=view==='habits'?<HabitsModule/>:view==='metrics'?<MetricsModule/>:view==='activity'?<ActivityModule manage={()=>setView('manage')}/>:view==='nutrition'?<NutritionModule/>:view==='sleep'?<SleepModule/>:view==='notes'?<NotesModule/>:<TrackModule onFirstSetupComplete={onFirstSetupComplete}/>;
  return <>
