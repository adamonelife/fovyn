...
const dayGroups:[HistoryData['items'][number]['kind'][],string][]=[[['record'],'Body & Metrics'],[['sleep'],'Sleep'],[['nutrition'],'Nutrition'],[['activity'],'Activity'],[['training'],'Training'],[['habit'],'Habits & Routines'],[['money'],'Money'],[['hobby'],'Hobbies'],[['social'],'Social'],[['alcohol'],'Alcohol'],[['recovery'],'Supplements & Recovery'],[['note'],'Notes']];
...
export default function HistoryModule({range=30,mode='calendar',openLog}:{range?:number;mode?:'calendar'|'timeline';openLog:()=>void}){
  ...
  return <div className={`history-content history-${mode}`}>
    ...
    <div className="timeline">{Object.entries(groups).map(([date,items])=><section className="day-group" key={date}>
      ...
      {items.map(item=><article className={`history-row ${item.kind==='training'?'history-openable':''}`} onClick={()=>item.kind==='training'&&setSelectedTraining(item.id)} key={`${item.kind}-${item.id}`}><span className={`record-icon ${item.kind}`}>{item.iconKey?<FunctionalIcon iconKey={item.iconKey}/>:<Activity/>}</span><div><b>{item.title}</b><small>{item.detail}{item.corrected?' · Corrected':''}</small></div><div className="linked-goals">{item.goalNames.map(goal=><span key={goal}>{goal}</span>)}</div><time>{item.kind==='training'?'Open':new Date(item.occurredAt).toLocaleTimeString(undefined,{hour:'2-digit',minute:'2-digit'})}</time></article>)}
    </section>)}
