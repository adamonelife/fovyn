...
export type SearchResult={id:string;type:'Goal'|'Habit'|'Logging item'|'Record';title:string;detail:string;destination:SearchDestination};

export async function loadSearchIndex():Promise<SearchResult[]>{
  const user=await goalOwner();
  const[goals,habits,trackers,records]=await Promise.all([
  ...
  fail('Search Goals',goals.error);fail('Search habits',habits.error);fail('Search logging items',trackers.error);fail('Search records',records.error);
