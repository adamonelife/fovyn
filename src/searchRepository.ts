...
export type SearchResult={id:string;type:'Goal'|'Habit'|'Logging item'|'Record'|'Note'|'Sleep'|'Activity'|'Nutrition'|'Money'|'Hobby';iconKey?:string|null;title:string;detail:string;destination:SearchDestination};

export async function loadSearchIndex():Promise<SearchResult[]>{
  const user=await goalOwner();
  const[goals,habits,trackers,records,notes,sleep,activities,nutrition,money,hobbyEntries,hobbies]=await Promise.all([
