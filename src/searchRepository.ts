...
export type SearchResult={id:string;type:'Goal'|'Habit'|'Logging item'|'Record'|'Note'|'Sleep'|'Activity'|'Nutrition'|'Money';title:string;detail:string;destination:SearchDestination};

export async function loadSearchIndex():Promise<SearchResult[]>{
  const user=await goalOwner();
  const[goals,habits,trackers,records,notes,sleep,activities,nutrition,money]=await Promise.all([
    ...
    supabase.from('money_transactions').select('id,transaction_type,amount,currency,note,occurred_at,corrected_at').eq('owner_id',user.id).is('deleted_at',null).order('occurred_at',{ascending:false}).limit(100)
  ]);
  fail('Search Goals',goals.error);fail('Search habits',habits.error);fail('Search logging items',trackers.error);fail('Search records',records.error);fail('Search Notes',notes.error);fail('Search Sleep',sleep.error);fail('Search Activities',activities.error);fail('Search Nutrition',nutrition.error);fail('Search Money',money.error);
