...
export type HistoryItem={id:string;kind:'record'|'habit'|'note'|'sleep'|'activity';title:string;detail:string;occurredAt:string;corrected:boolean;goalNames:string[]};
...
export async function loadHistory(days=30):Promise<HistoryData>{
  ...
  const items=[...recordItems,...habitItems,...noteItems,...sleepItems,...activityItems].sort((a,b)=>b.occurredAt.localeCompare(a.occurredAt)),dates=[...new Set(items.map(x=>x.occurredAt.slice(0,10)))];
  return{items,daysPresent:dates.length,currentStreak:streakForDates(dates),contributions:[...recordItems,...activityItems].filter(x=>x.goalNames.length>0).length};
}
