...
export type HistoryItem={id:string;kind:'record'|'habit'|'note'|'sleep'|'activity'|'nutrition';title:string;detail:string;occurredAt:string;corrected:boolean;goalNames:string[]};
...
export async function loadHistory(days=30):Promise<HistoryData>{
  ...
  let sleepQuery=supabase.from('sleep_entries').select('id,bedtime,wake_time,quality,waking_energy,corrected_at').eq('owner_id',user.id).is('deleted_at',null).order('wake_time',{ascending:false}).limit(500);if(since)sleepQuery=sleepQuery.gte('wake_time',since);const[sleep,sleepLinks]=await Promise.all([sleepQuery,supabase.from('sleep_entry_goals').select('sleep_entry_id,goals(title)').eq('owner_id',user.id)]);fail('History Sleep',sleep.error);fail('History Sleep links',sleepLinks.error);
  let activityQuery=supabase.from('cardio_entries').select('id,activity,duration_min,distance_km,is_social,occurred_at,performed_on,corrected_at').eq('owner_id',user.id).is('deleted_at',null).order('occurred_at',{ascending:false}).limit(500);if(since)activityQuery=activityQuery.gte('occurred_at',since);const[activities,activityLinks]=await Promise.all([activityQuery,supabase.from('cardio_entry_goals').select('cardio_entry_id,goals(title)').eq('owner_id',user.id)]);fail('History Activities',activities.error);fail('History Activity links',activityLinks.error);
  let nutritionQuery=supabase.from('nutrition_entries').select('id,name,meal_type,calories,protein_g,carbs_g,fat_g,fibre_g,occurred_at,corrected_at').eq('owner_id',user.id).is('deleted_at',null).order('occurred_at',{ascending:false}).limit(500);if(since)nutritionQuery=nutritionQuery.gte('occurred_at',since);const[nutrition,nutritionLinks]=await Promise.all([nutritionQuery,supabase.from('nutrition_entry_goals').select('nutrition_entry_id,goals(title)').eq('owner_id',user.id)]);fail('History Nutrition',nutrition.error);fail('History Nutrition links',nutritionLinks.error);
  ...
  const sleepItems:HistoryItem[]=(sleep.data??[]).map(s=>({id:s.id,kind:'sleep',title:'Sleep',detail:`${Math.round((new Date(s.wake_time).getTime()-new Date(s.bedtime).getTime())/36000)/100} hours · ${s.quality} · ${s.waking_energy} energy`,occurredAt:s.wake_time,corrected:Boolean(s.corrected_at),goalNames:(sleepLinks.data??[]).filter(x=>x.sleep_entry_id===s.id).flatMap(x=>{const goal=x.goals as unknown as {title:string}|null;return goal?[goal.title]:[]})}));
  ...
  const nutritionItems:HistoryItem[]=(nutrition.data??[]).map(n=>({id:n.id,kind:'nutrition',title:n.name,detail:`${n.meal_type} · ${n.calories} kcal · P ${n.protein_g}g · C ${n.carbs_g}g · F ${n.fat_g}g · Fibre ${n.fibre_g}g`,occurredAt:n.occurred_at,corrected:Boolean(n.corrected_at),goalNames:(nutritionLinks.data??[]).filter(x=>x.nutrition_entry_id===n.id).flatMap(x=>{const goal=x.goals as unknown as {title:string}|null;return goal?[goal.title]:[]})}));
  const items=[...recordItems,...habitItems,...noteItems,...sleepItems,...activityItems,...nutritionItems].sort((a,b)=>b.occurredAt.localeCompare(a.occurredAt)),dates=[...new Set(items.map(x=>x.occurredAt.slice(0,10)))];
  return{items,daysPresent:dates.length,currentStreak:streakForDates(dates),contributions:[...recordItems,...activityItems,...nutritionItems].filter(x=>x.goalNames.length>0).length};
}
