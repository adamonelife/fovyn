...
export type HistoryItem={id:string;kind:'record'|'habit'|'note'|'sleep';title:string;detail:string;occurredAt:string;corrected:boolean;goalNames:string[]};
...
export async function loadHistory(days=30):Promise<HistoryData>{
  ...
  const items=[...recordItems,...habitItems,...noteItems,...sleepItems].sort((a,b)=>b.occurredAt.localeCompare(a.occurredAt)),dates=[...new Set(items.map(x=>x.occurredAt.slice(0,10)))];
