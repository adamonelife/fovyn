...
export type HomeData={profile:{first_name:string|null;display_name:string|null;current_climate:string;onboarding_completed_at:string|null};habits:HomeHabit[];goals:HomeGoal[];recentCount:number;configuredCount:number};
...
export async function loadHome():Promise<HomeData>{
  const user=await goalOwner(),day=new Date().toISOString().slice(0,10),since=new Date(Date.now()-24*60*60*1000).toISOString();
  const[profile,goals,habits,recent,configured]=await Promise.all([
  ...
  fail('Configured items',configured.error);return{profile:profile.data,goals:(goals.data??[]) as HomeGoal[],recentCount:recent.count??0,configuredCount:configured.count??0,habits:(habits.data??[]).map(h=>{const s=(schedules.data??[]).find(x=>x.habit_id===h.id),e=(entries.data??[]).find(x=>x.habit_id===h.id);return{id:h.id,name:h.name,status:(e?.status??null) as HomeHabit['status'],frequency_type:(s?.frequency_type??null) as HomeHabit['frequency_type'],days_of_week:s?.days_of_week??[]}})};
}
