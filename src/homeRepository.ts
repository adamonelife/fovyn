...
import{forestEnvironment,forestHealth,forestSpecies,nurseryProgress}from'./forestGoalState';
...
export async function loadHome():Promise<HomeData>{
  ...
  const[goalContributions,goalRules,dormancy,forestOverrides,goalEvaluations]=goalIds.length?await Promise.all([
    ...
    supabase.from('goal_evaluation_periods').select('goal_id,status').in('goal_id',goalIds),
  ]):[{data:[],error:null},{data:[],error:null},{data:[],error:null},{data:[],error:null},{data:[],error:null}];
  fail('Forest contributions',goalContributions.error);fail('Forest Goal rules',goalRules.error);fail('Forest dormancy',dormancy.error);fail('Forest QA overrides',forestOverrides.error);fail('Forest Goal evaluations',goalEvaluations.error);
