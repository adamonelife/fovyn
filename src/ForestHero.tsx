...
import {usePublishedForestSlots} from './forestComposerRepository';
...
export default function ForestHero({goals,currentClearing,onViewGoal,onLog}:{goals:HomeGoal[];currentClearing:HomeClearing|null;onViewGoal:()=>void;onLog:()=>void}){
  ...
  const slots=usePublishedForestSlots(environment,forestEnvironmentSlots[environment]);
  ...
  return <section className={'production-forest-hero forest-'+environment+(clearingActive?' current-clearing':'')}>
    {background&&<ForestEnvironmentScene asset={background} className="production-forest-scene" overlay="linear-gradient(90deg,rgba(5,30,23,.66),rgba(5,30,23,.05) 55%,rgba(5,30,23,.2))">
